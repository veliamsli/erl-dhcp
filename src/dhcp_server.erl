...
offer_packet(Xid, LeaseInfo) ->
  #dhcp_packet{
    ...
    options = [{message_type, offer}, {server_id, {192, 168, 2, 4}}] ++ LeaseInfo#dhcp_lease.options
  }.
...
ack_packet(Xid, LeaseInfo) ->
  #dhcp_packet{
    ...
    siaddr = {192, 168, 2, 4},
    chaddr = LeaseInfo#dhcp_lease.chaddr,
    options = [{message_type, ack}, {server_id, {192, 168, 2, 4}}] ++ LeaseInfo#dhcp_lease.options
  }.
...
nak_packet(Packet) ->
  #dhcp_packet{
    ...
    options = [{message_type, nak}, {server_id, {192, 168, 2, 4}}]
  }.
